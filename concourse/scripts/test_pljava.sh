...
chmod 777 /tmp/*.sh
su - gpadmin -c "source $GPHOME/greenplum_path.sh && bash /tmp/gpdb_install.sh /data" || exit 1

# Installing PL/Java and running tests
su - gpadmin -c "bash /tmp/pljava_install_test.sh pljava_bin $OSVER $WORKDIR $TMPDIR"
RETCODE=$?
